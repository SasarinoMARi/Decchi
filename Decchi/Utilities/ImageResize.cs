...
namespace Decchi.Utilities
{
	...
	{
		public static Stream LoadImageResized(string path, int targetSize = 3145728) // 3 MB
		{
			var filesize = new FileInfo(path).Length;
			if (filesize < targetSize) return new FileStream(path, FileMode.Open, FileAccess.Read);
